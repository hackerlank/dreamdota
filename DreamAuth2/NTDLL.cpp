...
NtQueryInformationThreadT NtQueryInformationThread;
...
NTSTATUS QueryInformationThread(HANDLE handle, THREAD_INFORMATION_CLASS infoClass, QueryData* data) {
	jmp_buf env;
	int val = setjmp(env);
	NTSTATUS ntstatus;
	DWORD buffer_size = 4;
	QueryDataInit(data, buffer_size);
	ntstatus = NtQueryInformationThread(handle, infoClass, (PVOID)(data->buffer), buffer_size, &buffer_size);
	return ntstatus;
}
...
void Init() {
	...
	bool success = 
		...
		NULL != (NTDLL::NtQueryInformationThread = (NTDLL::NtQueryInformationThreadT)(GetProcAddress(hNtdll, "NtQueryInformationThread")));
