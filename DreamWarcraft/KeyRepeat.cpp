...
namespace KeyRepeat {
	...
	void onKeyDown (const Event *evt) {
		...
		VMProtectBeginVirtualization("KeyRepeatReadEnable");

		if (ProfileFetchInt("KeyAutoRepeat", "Enable", 1) <= 0) return;
	...
	void onFocus (const Event *evt) {
		uint32_t size = sizeof(RepeatTimer) / sizeof(Timer*);
		for (uint32_t keyCode = 0; keyCode < size; ++keyCode) {
			if (RepeatDelayTimer[keyCode]){
				RepeatDelayTimer[keyCode]->destroy();
				RepeatDelayTimer[keyCode] = NULL;
			}
	...
	void Init(){
		VMProtectBeginVirtualization("KeyRepeatInit");

		memset(RepeatTimer, NULL, sizeof(RepeatTimer));
		memset(RepeatDelayTimer, NULL, sizeof(RepeatDelayTimer));
		ProfileFetchInt("KeyAutoRepeat", "Enable", 1);
		//RepeatDelay = 0.3f;
		//RepeatRate = 30.f;
		RepeatDelay = ProfileFetchFloat("KeyAutoRepeat", "RepeatDelay", 0.3f);
		RepeatRate = ProfileFetchFloat("KeyAutoRepeat", "RepeatRate", 30.f);
