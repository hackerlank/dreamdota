...
namespace war3 {
	...
	typedef uint32_t HASHKEY_4CC;

	struct UnitDataNode {
		uint32_t	unk_0;			//0x0
		uint8_t		unk_4[0x10];	//0x4
		uint32_t	typeId;			//0x14
		int			buildTime;		//0x18
		...
		uint32_t	unk_290;		//0x290
		uint32_t	unk_294;		//0x294
		uint32_t	unk_298;		//0x298
		RCString	str_29C;		//0x29C
		RCString	str_2A8;		//0x2A8
		uint32_t	unk_2B4;		//0x2B4
	};//sizeof = 0x2BC
	...
	struct CGxDeviceD3d {
		...
		uint32_t		unk_6D0;			//0x6D0 窗口大小改变，这里设为1
		//..
	};
	...
	struct CGameState {
		...
		CTextTagManager		textTagManager; //0x2C8
	};//sizeof?
	...
	struct CGameWar3 {
		...
		uint32_t		maxPlayers;			//+44 - Max player slots on the map.
		uint8_t			unk_48[0x04];
		uint32_t		activePlayers;		//+4C - Active players.
		uint8_t			unk_50[0x08];
	...
	struct CSelectionWar3 {
		...
		uint32_t			playerId;			//0x1AC
		uint32_t			unk_1B0[5];
		UnitList			unitSelectedSync;	//0x1C4
		CUnit*				unit_b1;			//0x1D8
		uint32_t			unit_id;			//0x1DC
	...
	struct CUnit {//TODO +0x14C -1 
		...
		HashGroup					hash;				//0xC
		uint8_t unk_14[0xC];
		uint32_t					stateFlag;			//0x20 表示状态, 1 << 3 表示无敌 TODO 继续研究其他bit
		uint8_t unk_24[0xC];
	...
	struct CItem {
		...
		uint32_t		unkFlag_5C;			//0x5C

		uint8_t			unk_60[0x28];		//0x60

		uint32_t		firstAbilityId;		//0x88
	...
	struct AbilityUIDef {
		...
		uint32_t	hotkey_levels;	//0x80
		uint32_t*	hotkey_items;	//0x84
		uint32_t	unk_88;			//0x88
		uint32_t	hotkey2_levels;	//0x8C
		uint32_t*	hotkey2_items;	//0x90
		uint8_t		unk_94[0xA0];	//0x94
		uint32_t	tooltip_levels;	//0x134
		char**		tooltip_items;	//0x138
		uint32_t	unk_13C;		//0x13C
		uint32_t	tooltip2_levels;//0x140
		char**		tooltip2_items;	//0x144		
		uint8_t		unk_148[0x10];	//0x148
		uint32_t	desc_levels;	//0x158
		char**		desc_items;		//0x15C
		uint32_t	unk_160;		//0x160
		uint32_t	desc2_levels;	//0x164
	...
	struct AbilityDefData {
		...
		uint32_t uiDefAvailable;	//0x2C
		uint32_t abilityBaseTypeId;	//0x30
		//...
	};//sizeof = ?
	...
	struct CAbility {
		...
		uint32_t unk18;
		uint32_t unk1C;
		uint32_t		flag2;			//0x20	& 0x200 表示是否在CD中
	...
	struct CAbilityQueue
	{
		...
		HashGroup		hashPoReal;		//0x84
		uint8_t unk_8C[0x14];
		uint32_t		queueSlotCount;	//0xA0
		uint32_t		queueItemType[7];
		//...
	};

	struct CAbilitySpell {
		CAbility		baseAbility;	//0x0
		uint32_t		unk6C;			//0x6C = dword_6F92ED9C
		uint32_t		unk70;
		uint32_t		unk74;
		uint32_t		unk78;
		uint32_t		unk7C;
		...
		uint32_t		unkA0;			// = 0xFFFFFFFF
		uint32_t		unkA4;			// = 0xFFFFFFFF
		uint32_t		unkA8;			// = 0
		uint32_t		unkAC;			// = 0
	...
	struct CAbilityAttack {
		...
		uint8_t unk_70 [0x14];
		uint32_t			dice_weap0;		//0x88
		uint32_t			dice_weap1;		//0x8C
		uint8_t unk_90 [0x4];
		uint32_t			sides_weap0;	//0x94
		uint32_t			sides_weap1;	//0x98
		uint8_t unk_9C [0x4];
		uint32_t			dmgPlus_weap0;	//0xA0
		uint32_t			dmgPlus_weap1;	//0xA4
		uint8_t unk_A8 [0x4];
		uint32_t			buffs_weap0;	//0xAC
		uint32_t			buffs_weap1;	//0xB0
		uint8_t unk_B4 [0x28];
		uint32_t			weapTp_weap0;	//0xDC
		uint32_t			weapTp_weap1;	//0xE0
		uint8_t unk_E4 [0x10];
		uint32_t			atkType_weap0;	//0xF4
		uint32_t			atkType_weap1;	//0xF8
		uint8_t unk_FC [0x58];
		FloatMini			cool_weap0;		//0x154
		FloatMini			cool_weap1;		//0x15C
		uint32_t unk_164;					//			TODO
		FloatMiniB			dmgPt_weap0;	//0x168
		FloatMiniB			dmgPt_weap1;	//0x178
		uint32_t unk_188;					//			TODO
	...
	struct CBulletBase {
		...
		uint32_t			weaponSound;	//0x58	武器声音 Jass::WEAPON_TYPE_X
		uint32_t			attackType;		//0x5C	攻击类型 Jass::ATTACK_TYPE_X
		uint32_t unk_60;					//0x60	flag，值通常=00 01 00 00，第一个byte改为0xFF会无伤害	TODO 研究一下攻击miss与暴击
	...
	struct CMissileLine {
		CMissile			baseMissile;
		uint32_t			typeId;			//0xAC 攻击者的单位类型id ?
		uint32_t unk_B0;
		uint32_t unk_B4;	//某种flag
	...
	struct MapData	{
		uint32_t			unk_0;			//0x0	= 1?
		uint32_t			unk_4;			//0x4	= 1?
		uint32_t			unk_8;			//0x8	= 1?

		uint32_t			v1a;			//0xC	例如 37280
		uint32_t			v1b;			//0x10	例如 37249
		MapDataBlock*		dataBlocks1;	//0x14	应该是地图数据
		uint32_t			unk_18;			//0x18	例子 32

		uint32_t			v2a;			//0x2C	例子		7075
		uint32_t			v2b;			//0x30			7065
		MapDataBlock*		dataBlocks2;	//0x34	应该是地图数据
		uint32_t			unk_38;			//0x38	例子 25

		uint32_t			v3a;			//0x3C	例子		4608
		uint32_t			v3b;			//0x40			4382
		MapDataBlock*		dataBlocks3;	//0x44	应该是地图数据
		uint32_t			unk_48;			//0x48	例子 256

		uint32_t			unk_4C;			//0x4C	FFFFFF80
		uint32_t			unk_50;			//0x50	例子 300
		uint32_t			unk_54;			//0x54	-1
		uint32_t			unk_58;			//0x58	0
		uint32_t			counter;		//0x5C	53030402
		//...
	};
	...
	struct MapInfo {
		...
		CSpawn*					spawn;				//0x2C
		CTextTagManager*		textTagManager;		//0x30
		void* unk_34;
	...
	struct CNetData {
		...
		uint32_t			replayState;		//0x614, 录像模式=2 ?
		uint8_t				unk_618[0x1658];//0x618
		uint32_t			time;			//0x1C70
		uint8_t				unk_1C74[0x640];		//0x1C74
		uint32_t			replaySpeedFactorNumerator;	//0x22B4
		uint32_t			replaySpeedFactorDenominator; //0x22B8
		//..
	};
	...
	// Event
	//--------------------------------------------------------------------------------
	const uint32_t EVENT_FOCUS			= 0x40040065;

	const uint32_t EVENT_KEYDOWN		= 0x40060064;
	const uint32_t EVENT_KEYUP			= 0x40060066;
	const uint32_t EVENT_CHAR			= 0x40060067;

	const uint32_t EVENT_MOUSE_DOWN		= 0x400500C8;
	const uint32_t EVENT_MOUSE_UP		= 0x400500C9;
	const uint32_t EVENT_MOUSE_MOVE		= 0x400500CA;
	const uint32_t EVENT_MOUSE_SCROLL	= 0x400500CD;
	...
	*/
	...
	const uint32_t EVENT_ABILITY_CLICK			= 0x00030064; //当点击的是技能栏中的按钮时
	const uint32_t EVENT_ITEM_CLICK				= 0x00030065; //当点击的是物品栏中的按钮时
	const uint32_t EVENT_TRAINABLE_CLICK		= 0x00030066; //当点击的是训练中的按钮时
	const uint32_t EVENT_MINIMAPSIGNAL_CLICK	= 0x00000015; //Minimap Signal
	const uint32_t EVENT_MINIMAPTERRAIN_CLICK	= 0x00000016; //Minimap Terrain
	const uint32_t EVENT_ALLYCOLORMODE_CLICK	= 0x00000017; //Minimap AllyColor
	const uint32_t EVENT_MINIMAPCREEP_CLICK		= 0x00000018; //Minimap Creep
	const uint32_t EVENT_FORMATION_CLICK		= 0x0000001B; //Formation Movement

	const uint32_t EVENT_GAMETICK				= 0x40090092;
	const uint32_t EVENT_GAMEIDLETICK			= 0x40090093;
	const uint32_t EVENT_GAMEASYNCIDLETICK		= 0x40090094;
	const uint32_t EVENT_GAMESTART				= 0x4009007E;
	const uint32_t EVENT_GAMEREADY				= 0x4009007F;
	const uint32_t EVENT_GAMELEAVE				= 0x40090081;

	const uint32_t EVENT_PLAYER_CHAT			= 0x80210;

	//CD结束 = 0xD01B0
	const uint32_t EVENT_UNIT_ATTACK_RELEASED	= 0xD01B1;
	const uint32_t EVENT_UNIT_ACQUIRE_START		= 0xD0163;
	const uint32_t EVENT_UNIT_ACQUIRE_READY		= 0xD01AF;
	const uint32_t EVENT_UNIT_ACQUIRE_REFRESH	= 0xD0144;
	...
	struct CMouseEvent {
		CEvent			baseEvent;		//0x0
		uint32_t unk_10;
		uint32_t		keyCurrent;		//0x14 按下事件为有效按下键值，弹起事件为有效弹起键值
		uint32_t		keyFlag;		//0x18 按下事件为所有按下键值，弹起事件为剩余按下键值，拖拽事件为拖拽键值
		uint32_t unk_1C;
		uint32_t unk_20;
	...
	struct CSimpleButtonClickEvent {
		...
		uint32_t		keyFlag;		//0x10	同MouseEvent,但是应该只响应左右键
	/*
		注：以下实际上不属于event内容
	...
	*/
	...
	struct PacketAction {
		...
		uint32_t		actionId;			//0x3	order id like 0xD0004 or 'ewsp'
	...
	struct PacketActionPoint {
		...
		uint16_t		flag;				//0x1	ability flag
		uint32_t		actionId;			//0x3	order id
		HashGroup		hashUsedItem;		//0x7	hash group of item used
		float			targetX;			//0xF
		float			targetY;			//0x13
	};//sizeof = 23
	...
	struct PacketActionIndirect {
		...
		float			targetX;			//0xF
		float			targetY;			//0x13
		uint32_t		typeId;				//0x17	type id of target
		uint8_t			unk_1F[9];
		float			targetX2;			//0x24
		float			targetY2;			//0x28
	};//sizeof = 44
	...
	struct PacketSubgroup {
		...
		uint32_t		typeId;				//0x1	type id of new active subgroup
	...
	struct PacketCancelQueue {
		...
		uint32_t		typeId;				//0x2	type id of canceled object
	};//sizeof = 6
