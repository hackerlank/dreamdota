...
namespace InfoPanel {
	...
	const char* RaceIconGet(race inRace)
	{
		switch (inRace)
		{
		case Jass::RACE_HUMAN:
			return "DreamDota3\\Textures\\raceicon_human.tga";
		case Jass::RACE_NIGHTELF:
			return "DreamDota3\\Textures\\raceicon_nightelf.tga";
		case Jass::RACE_ORC:
			return "DreamDota3\\Textures\\raceicon_orc.tga";
		case Jass::RACE_UNDEAD:
			return "DreamDota3\\Textures\\raceicon_undead.tga";
		default:
			return "UI\\Glues\\ScoreScreen\\scorescreen-player-naga.blp";
		}
	}
		...
		itemCount(inItemCount), height(inHeight), itemWidth(inItemWidth), hidden(false), width(0)
	{
		...
		playerColorBg = UISimpleTexture::Create(frame);
		playerColorBg->setHeight(raceIconSize * 0.45f);
		playerColorBg->setWidth(raceIconSize * 0.83f);
		playerColorBg->fillColor(0xFFFF0000);
		playerColorBg->setRelativePosition(
			POS_L,
			frame,
			POS_L,
			0.0035f,
		...
		playerRaceIcon = UISimpleTexture::Create(playerIconFrame);
		playerRaceIcon->setHeight(raceIconSize);
		playerRaceIcon->setWidth(raceIconSize);
		playerRaceIcon->fillBitmap(RaceIconGet(PlayerRace(playerId)));
		playerRaceIcon->setRelativePosition(
			POS_L,
			frame,
			POS_L,
			0.002f,
	...
	void Row::update()//MUSTDO Æ¥Åä¿í¶È
	{
		...
		if (playerWantUpdate)
		{
			Color color = GetPlayerColor(playerId, false);
			playerColorBg->fillColor(color.toUint32());
			playerRaceIcon->fillBitmap(RaceIconGet(PlayerRace(playerId)));
