...
ObjectPool<UISimpleFrame> SimpleFramePool(20);
...
UISimpleFrame* UISimpleFrame::Create(UISimpleFrame* UIParent) {
	...
	aero::generic_this_call<war3::CSimpleFrame*>(
...
void UISimpleFrame::show() {
	...
	if (baseObj) {
		...
		if (func)
		{
			aero::generic_this_call<void>(func, baseObj);
		}
	}
}
...
void UISimpleFrame::setParent(UISimpleFrame* parent) {
	...
	if ((parentBaseObj = parent->base<war3::CSimpleFrame*>()) && parent != this) {
		aero::generic_this_call<uint32_t>(Offset(SIMPLEFRAME_SETPARENT), baseObj, parentBaseObj);
	}
}
...
void UISimpleFrame::initTextureSettings() {
	...
	if (baseObj) {
		if (!baseObj->textureSettings) {
			baseObj->textureSettings =  Storm::MemAllocStruct<war3::SimpleFrameTextureSettings>();
			baseObj->textureSettings = aero::generic_fast_call<war3::SimpleFrameTextureSettings*>(
