...
UISimpleStatusBar* UISimpleStatusBar::Create(UISimpleFrame* parent) {
	...
	war3::CSimpleButton* baseObj = Storm::MemAllocStruct<war3::CSimpleButton>();
...
void UISimpleStatusBar::setValue(float value) {
	...
	if (baseObj) {
		aero::generic_this_call<void>(
			aero::offset_element_get<void*>(VTBL(baseObj), 0x6C),
			baseObj,
			value
		);
	}
...
void UISimpleStatusBar::setTexture(const char* path) {
	...
	if (baseObj) {
		...
		if (NULL != (texture = baseObj->texture)) {
			...
			texture = baseObj->texture = Storm::MemAllocStruct<war3::CSimpleTexture>();
			if (texture) {
				aero::generic_this_call<war3::CSimpleTexture*>(
