...
void CheckBoxEventHandler(Observer* ob, uint32_t evtId) 
{
...
void CheckBox::check( bool flag )
{
	...
	//{
		...
		{
			ProfileSetBool( boundProfileApp.c_str(), boundProfileKey.c_str(), bIsChecked );
		}
	//}
...
void CheckBox::bindProfile( const char *appName, const char *keyName, bool defaultChecked /*= true */ )
{
	...
	if (boundProfileApp.length() && boundProfileKey.length())
	{
		bool wantCheck = ProfileFetchBool(boundProfileApp.c_str(), boundProfileKey.c_str(), defaultChecked);
		check(wantCheck);
	}
	else
	{
